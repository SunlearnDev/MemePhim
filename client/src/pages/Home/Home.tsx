      ...
      <div className="main-content bg-black-2">
        ...
        <section>
          <Wrapper className="mt-2">
            <h2 className='text-light-gray py-1 text-2xl relative after:content-[""] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-[1px] after:bg-white/40'>
              Top Trending{" "}
            </h2>
            {trendingQuery.data && (
              <ListMovieHorizontal
        ...
        <section>
          <Wrapper>
            <h2 className="text-light-gray flex-col xs:flex-row gap-x-6 gap-y-4 flex  py-1 text-2xl relative  ">
              <span className='after:content-[""] relative after:absolute after:bottom-0 pb-1 after:left-0 after:w-16 after:h-[1px] after:bg-white/40'>
                Top Rating
              </span>
              <div className="flex">
                <button
                  ...
                  className={classNames(
                    "flex  items-center justify-center text-xs gap-1 font-medium bg-blue-gray hover:bg-blue-gray-2 transition duration-300  p-2 rounded [&.active]:bg-dark-teal [&.active]:text-white ",
                    { active: topRatingSelect === "movie" }
                  )}
                >
                ...
                <button
                  ...
                  className={classNames(
                    "flex ml-2 items-center justify-center text-xs gap-1 font-medium bg-blue-gray hover:bg-blue-gray-2 transition duration-300  p-2 rounded [&.active]:bg-dark-teal [&.active]:text-white ",
                    { active: topRatingSelect === "tv" }
                  )}
                >
                  <MdLiveTv className="text-xl" />{" "}
                  <span className="mt-0.5">TV-Series</span>
                </button>
              </div>
            </h2>
            {topRatedQuery.data && (
              <ListMovieHorizontal
                mediaType={topRatingSelect}
                className="pb-8 pt-6"
                data={
                  (topRatedQuery.data.data.results as Movie[]) ||
                  (topRatedQuery.data.data.results as TV[]) ||
                  []
                }
              />
            )}
        ...
        <section>
          <Wrapper>
            ...
            {popularQuery.data && (
              <ListMovieHorizontal
                mediaType={popularSelect}
                className="pb-8 pt-6"
                data={
                  (popularQuery.data.data.results as Movie[]) ||
                  (popularQuery.data.data.results as TV[]) ||
                  []
                }
              />
            )}
        ...
        <section className="top-rated py-6 bg-black-2">
          <Wrapper>
            <h2 className='text-light-gray flex py-1 text-2xl relative after:content-[""] after:absolute after:bottom-0 after:left-0 after:w-16 after:h-[1px] after:bg-white/40'>
              ...
              <Link
                to={"/movies"}
                className="flex ml-auto items-center text-sm text-dark-teal transition-opacity duration-300 hover:opacity-75"
              >
                View all{" "}
                <span className="text-base bg-dark-teal rounded-full text-black ml-2">
            ...
            <GridContainer className="lg:gap-x-3 gap-y-6 gap-x-2">
              {latestMovieQuery.data &&
                latestMovieQuery.data.data.results.map((movie, index) => {
                  return (
                    <Card
              ...
              {latestMovieQuery.isLoading &&
                new Array(14).fill(0).map((_, index) => {
                  return <SkeletonCard key={index.toString() + "movies"} />;
                })}
            </GridContainer>
          </Wrapper>
        </section>
        ...
        <section className="top-rated py-6 bg-black-2">
          <Wrapper>
            ...
            <GridContainer className="lg:gap-x-3 gap-y-6 gap-x-2">
              {latestTVQuery.data &&
                latestTVQuery.data.data.results.map((tv, index) => {
                  if (!tv.poster_path) return;
                  return (
                    <Card
                      key={tv.id + `-${Math.random().toString()}`}
                      mediaType="tv"
                      data={tv}
                    />
                  );
              ...
              {latestTVQuery.isLoading &&
                new Array(14).fill(0).map((_, index) => {
                  return <SkeletonCard key={index.toString() + "tv-series"} />;
                })}
            </GridContainer>
          </Wrapper>
        </section>
